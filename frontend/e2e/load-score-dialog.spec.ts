...
test.describe('Feature 028: Load Score Dialog', () => {
  ...
  test('all preloaded score files are fetched at a correct URL (no 404)', async ({ page }) => {
    // We only need to verify ONE fetch URL: all 6 paths are generated by the
    // same formula (import.meta.env.BASE_URL + 'scores/' + filename), so if
    // one resolves correctly every other will too. Multi-click approaches are
    // brittle because the dialog disables items while busy / in error state.
    let fetchedUrl: string | null = null;
    await page.route('**/*.mxl', async (route) => {
      fetchedUrl = route.request().url();
      // Fulfill immediately so the app doesn't hang waiting for the network.
      // Empty body → WASM parse error (expected), but the URL is captured.
      await route.fulfill({ status: 200, body: '', contentType: 'application/octet-stream' });
    });

    await page.getByRole('button', { name: /load score/i }).click();

    // Click the first score and wait for its .mxl fetch to fire
    const requestPromise = page.waitForRequest('**/*.mxl', { timeout: 5000 });
    await page.getByText('Bach — Invention No. 1').click();
    await requestPromise;

    // The URL must include the correct scores path.
    // On GitHub Pages (VITE_BASE=/musicore/): …/musicore/scores/Bach_InventionNo1.mxl
    // Locally (VITE_BASE=/):                  …/scores/Bach_InventionNo1.mxl
    expect(fetchedUrl, 'Score URL missing expected path segment').toMatch(
