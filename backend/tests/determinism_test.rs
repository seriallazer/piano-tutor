...
fn test_deterministic_layout_violin() {
    ...
    let json_content =
        fs::read_to_string(&fixture_path).expect("Failed to read violin_10_measures.json");
...
fn test_deterministic_layout_piano() {
    ...
    let json_content =
        fs::read_to_string(&fixture_path).expect("Failed to read piano_8_measures.json");
