...
fn test_multi_staff_instrument_independence() {
    ...
    bass_staff
        ...
        .push(musicore_backend::domain::events::staff::StaffStructuralEvent::Clef(bass_clef));
    ...
    let first_staff_clefs: Vec<_> = instrument.staves[0]
        .staff_structural_events
        .iter()
    ...
    let second_staff_clefs: Vec<_> = instrument.staves[1]
        .staff_structural_events
        .iter()
        .filter_map(|e| match e {
            musicore_backend::domain::events::staff::StaffStructuralEvent::Clef(ce) => Some(ce),
...
fn test_score_get_tempo_at_tick() {
    ...
    assert_eq!(
        tempo_at_0.unwrap().bpm,
        musicore_backend::domain::value_objects::BPM::new(120).unwrap()
    );
