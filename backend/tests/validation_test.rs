use musicore_backend::domain::{
    ...
    score::Score,
    staff::Staff,
    value_objects::{BPM, Tick, Clef, KeySignature},
};
...
fn test_score_prevent_delete_tempo_at_tick_0() {
    ...
    assert!(matches!(result.unwrap_err(), musicore_backend::domain::errors::DomainError::ConstraintViolation(_)));
}
...
fn test_score_delete_nonexistent_tempo() {
    ...
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), musicore_backend::domain::errors::DomainError::NotFound(_)));
}
...
fn test_score_query_events_in_range() {
    ...
    score.add_tempo_event(TempoEvent::new(Tick::new(960), BPM::new(140).unwrap())).unwrap();
    score.add_tempo_event(TempoEvent::new(Tick::new(3840), BPM::new(100).unwrap())).unwrap();
    score.add_time_signature_event(TimeSignatureEvent::new(Tick::new(1920), 3, 4)).unwrap();
...
fn test_staff_query_events_in_range() {
    ...
    // Add events at various ticks
    staff.add_clef_event(ClefEvent::new(Tick::new(960), Clef::Bass)).unwrap();
    staff.add_clef_event(ClefEvent::new(Tick::new(3840), Clef::Alto)).unwrap();
    staff.add_key_signature_event(KeySignatureEvent::new(Tick::new(1920), KeySignature::new(2).unwrap())).unwrap();
