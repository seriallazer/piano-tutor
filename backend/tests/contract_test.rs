...
fn test_violin_fixture_contract() {
    ...
    let fixture_content = fs::read_to_string(&fixture_path)
        .expect("Failed to read violin_10_measures.json fixture");
    
    let expected_output: serde_json::Value = serde_json::from_str(&fixture_content)
        .expect("Failed to parse violin fixture JSON");
    ...
    let actual_output = musicore_backend::layout::compute_layout(
        &test_input,
        &config
    );

    // Serialize to JSON for comparison
    let actual_json = serde_json::to_value(&actual_output)
        .expect("Failed to serialize layout output");
    ...
    assert!(
        !systems.is_empty(),
        "Systems array should not be empty"
    );
    ...
    let staves = first_staff_group["staves"].as_array()
    ...
    let staff_lines = first_staff["staff_lines"].as_array()
    ...
    assert_eq!(y_positions, vec![0.0, 40.0, 80.0, 120.0, 160.0], 
               "Staff lines should be at 0, 40, 80, 120, 160 (40-unit spacing)");

    // Verify glyph_runs exist (noteheads)
    let glyph_runs = first_staff["glyph_runs"].as_array()
...
fn test_piano_fixture_contract() {
    ...
    let fixture_content = fs::read_to_string(&fixture_path)
        .expect("Failed to read piano_8_measures.json fixture");
    
    let expected_output: serde_json::Value = serde_json::from_str(&fixture_content)
        .expect("Failed to parse piano fixture JSON");
