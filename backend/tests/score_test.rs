use musicore_backend::domain::{
    events::{tempo::TempoEvent, time_signature::TimeSignatureEvent},
...
fn test_score_default_time_signature_at_tick_0() {
    ...
    let time_sig_events: Vec<_> = score
        .global_structural_events
        .iter()
        .filter_map(|e| match e {
            musicore_backend::domain::events::global::GlobalStructuralEvent::TimeSignature(te) => {
                Some(te)
            }
            _ => None,
        })
...
fn test_score_add_tempo_event() {
    ...
    let tempo_events: Vec<_> = score
        ...
        .filter(|e| {
            matches!(
                e,
                musicore_backend::domain::events::global::GlobalStructuralEvent::Tempo(_)
            )
        })
        .collect();
