...
import type { BoundingBox, Point, System, Glyph, GlobalLayout, TickRange } from "./GlobalLayout";
...
export function getVisibleSystems(layout: GlobalLayout, viewport: BoundingBox): System[] {
  return layout.systems.filter((system) => intersects(system.boundingBox, viewport));
}
 ...
 *   if (glyph) {
 *     highlightNote(glyph.sourceReference);
 *   }
 * });
 */
export function findGlyphAtPosition(
  ...
  for (const system of systemsToSearch) {
    if (!containsPoint(system.boundingBox, clickPos)) {
      continue;
    }
    ...
    for (const staffGroup of system.staffGroups) {
      for (const staff of staffGroup.staves) {
        ...
        for (const glyph of staff.structuralGlyphs) {
          if (containsPoint(glyph.boundingBox, clickPos)) {
            return glyph;
          }
        }
...
export function findSystemAtTick(layout: GlobalLayout, tick: number): System | undefined {
  ...
  while (left <= right) {
    ...
    if (tick < system.tickRange.startTick) {
      right = mid - 1;
    } else if (tick >= system.tickRange.endTick) {
      left = mid + 1;
    } else {
 ...
 * 
 * @example
 * const pixelX = logicalToPixels(glyph.position.x, layout.unitsPerSpace, 12);
 */
 ...
 * 
 * @example
 * const logicalPos = pixelsToLogical(e.offsetX, layout.unitsPerSpace, 12);
 */
...
export function containsTickRange(outer: TickRange, inner: TickRange): boolean {
  return inner.startTick >= outer.startTick && inner.endTick <= outer.endTick;
}
 ...
 * 
 * @example
 * const duration = formatTickRangeDuration({ startTick: 0, endTick: 3840 });
...
export function countGlyphs(layout: GlobalLayout): number {
  ...
  for (const system of layout.systems) {
    for (const staffGroup of system.staffGroups) {
      for (const staff of staffGroup.staves) {
        count += staff.structuralGlyphs.length;
        for (const run of staff.glyphRuns) {
