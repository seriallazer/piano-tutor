...
- [x] T095 Fix cross-system incoming tie/slur arcs: incoming arcs started at `unified_left_margin` which equals or exceeds the first note's x position minus notehead width, creating backward (invisible) arcs; changed start to `unified_left_margin - 3*notehead_half_w` with min-span guarantee so incoming arcs are always visible- [x] T116 Fix courtesy/editorial accidentals not rendering in browser: `convertScoreToLayoutFormat()` in `frontend/src/components/layout/LayoutView.tsx` was not forwarding `has_explicit_accidental` to the WASM layout engine; the backend emits the flag in JSON (skip_serializing_if = false) and extraction.rs reads it, but the frontend mapping stripped it; added `has_explicit_accidental?: boolean` to `Note` interface in `frontend/src/types/score.ts` and forwarded it in `LayoutView.tsx`; added E2E regression test in `frontend/e2e/m21-flat-check.spec.ts` (Chopin Nocturne M21 Eb4 courtesy flat); confirmed via Playwright console logs that flat at x=2193.4, y=4620 now appears in WASM layout output- [x] T096 Fix slur rendering quality: endpoints repositioned to notehead edge (Y) and near centre (X); stroked line replaced with tapered filled crescent (two Bézier curves forming a lens shape); control-point X offset reduced from 33% to 20% of span; arc height changed from linear to `3.5*sqrt(span)` clamped [12, 50] for consistent curvature across all span lengths
...
- [x] T115 Implement ottava (8va/8vb) notation: parse `<octave-shift>` elements from MusicXML in `backend/src/domain/importers/musicxml/{types,parser,converter}.rs`; store `OctaveShiftRegion` on `Score` with start/end tick, display_shift, and staff_index; inject regions into `StaffData.octave_shift_regions` in layout extraction; transpose note display pitches (−12 semitones for 8va) before layout processing in `backend/src/layout/mod.rs`; add `OttavaBracketLayout` struct to `backend/src/layout/types.rs` and populate it per system with correct x/y positions; add `ottava_bracket_layouts` to `OttavaBracketLayout` in `frontend/src/wasm/layout.ts`; add `OctaveShiftRegion` type to `frontend/src/types/score.ts`; forward regions in `frontend/src/components/layout/LayoutView.tsx`; render "8va" label + dashed horizontal line + vertical hooks in `frontend/src/components/LayoutRenderer.tsx`; affected scores: Beethoven Für Elise M82-M83 and Chopin Nocturne M31-M36
- [x] T124 Fix LH/RH alignment regression from M9 onwards (Chopin Nocturne): `convert_note()` in `converter.rs` computed the correct tick for the principal note after grace notes but failed to rewind `timing_context.current_tick`; subsequent notes in the same voice accumulated a cumulative tick offset equal to the total grace_tick_advance, causing treble staff ticks to diverge from bass staff ticks; restored the original `current_tick` rewind logic so all notes after grace groups resume from the correct beat position
