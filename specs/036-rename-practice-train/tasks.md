...
- [X] T040 Update E2E spec `frontend/e2e/practice-view.spec.ts` → rename to `train-view.spec.ts`; replace all `/practice/i` button/heading selectors with `/train/i`; replace `data-testid="practice-view"` with `data-testid="train-view"`; replace `/play score/i` back-navigation assertion with `/play/i` (display name changed in T039)
- [X] T041 Update E2E spec `frontend/e2e/practice-virtual-keyboard.spec.ts` → rename to `train-virtual-keyboard.spec.ts`; update selector constants (`PRACTICE_BTN`, `PRACTICE_VIEW`, `PLAY_BTN`, `STOP_BTN`, `VKB_CONTAINER`) to Train equivalents; replace `.practice-mic-badge--suspended` with `.train-mic-badge--suspended`; replace `.practice-vkb__key--white` with `.train-vkb__key--white`; rename helper function `openPractice` → `openTrain`
