...
- [ ] T012 [US3] Update `frontend/plugins/practice-view-plugin/PracticeViewPlugin.tsx` to import `ResultsOverlay` and replace inline results JSX with `<ResultsOverlay ... />` component usage. Keep `isReplaying` and `replayHighlightedNoteIds` state in orchestrator, pass as props. Wire `onRepractice` to `handlePracticeToggle` and `onDismiss` to close overlay.
- [ ] T013 [US3] Create smoke test in `frontend/plugins/practice-view-plugin/ResultsOverlay.test.tsx` — render with mock props and verify it mounts without errors, renders results container element
- [ ] T014 [US3] Run full test suite (`cd frontend && npx vitest run`) — all tests must pass. Commit: `refactor(practice): extract ResultsOverlay component`
