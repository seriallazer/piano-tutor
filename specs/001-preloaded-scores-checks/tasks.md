...
- [x] T007 [US1] Review Burgmüller La Candeur: open score in browser, compare against `specs/050-fix-layout-preloaded-scores/references/Burgmuller_LaCandeur.png`, complete all checklist items per `specs/001-preloaded-scores-checks/contracts/review-protocol.md`, create `specs/001-preloaded-scores-checks/reviews/01-LaCandeur-final.md`
- [x] T008 [US1] Review Burgmüller Arabesque: open score, compare against `specs/050-fix-layout-preloaded-scores/references/Burgmuller_Arabesque.png`, complete checklist, create `specs/001-preloaded-scores-checks/reviews/02-Arabesque-final.md`
- [x] T009 [US1] Review Pachelbel Canon in D: open score, compare against `specs/050-fix-layout-preloaded-scores/references/Pachelbel_CanonD.png`, complete checklist, create `specs/001-preloaded-scores-checks/reviews/03-CanonD-final.md`
- [x] T010 [US1] Review Bach Invention No. 1: open score, compare against `specs/050-fix-layout-preloaded-scores/references/Bach_InventionNo1.png`, complete checklist, create `specs/001-preloaded-scores-checks/reviews/04-Invention-final.md`
- [x] T011 [US1] Review Beethoven Für Elise: open score, compare against `specs/050-fix-layout-preloaded-scores/references/Beethoven_FurElise.png`; pay specific attention to grace notes (M26), slur concavity (M52), augmentation dot de-collision on chord seconds (M63/M69), ottava bracket (M82–M83); complete checklist, create `specs/001-preloaded-scores-checks/reviews/05-FurElise-final.md`
- [x] T012 [US1] Review Chopin Nocturne Op. 9 No. 2: open score, compare against `specs/050-fix-layout-preloaded-scores/references/Chopin_NocturneOp9No2.png`; complete the standard checklist PLUS all Nocturne-specific checks from `contracts/review-protocol.md` (M29 𝄫 vs ♮, M30 8va bracket, M34–M36 accidentals and rest centering, M37 slur, M29–M37 LH/RH alignment); create `specs/001-preloaded-scores-checks/reviews/06-Nocturne-final.md`

### Remediation (Conditional — Execute Only If a Score Is Rejected)

- [x] T013 [US1] If any score review returns `REJECTED`: write a failing regression test reproducing the issue (Constitution VII — test MUST fail before fix is applied); apply targeted fix; run `cargo test` + `npm run test -- --run` — full suite must pass; rebuild WASM (T005); re-review the affected score only and update its approval record

  **Executed remediations** (issues found during Nocturne review and fixed before sign-off):

  - [x] T013-A Fix M2 LH staccato dot placement (dots below noteheads): root cause — `convertScoreToLayoutFormat()` in `LayoutView.tsx` was silently dropping the `stem_down` field, so annotation engine received `explicit_stem=None` and fell back to wrong side. Fix: add `stem_down?: boolean` to `Note` interface in `score.ts`; forward from `LayoutView.tsx`; regression test `frontend/e2e/nocturne-m2-staccato-verify.spec.ts` added. Commits: `b83b4f5`, `f79bc38`
  - [x] T013-B Fix M36 32nd note stem direction (Nocturne, notes 9-12 stems wrong): root cause — `position_glyphs_for_staff()` in `note_layout.rs` never consulted `NoteEvent.stem_down` for beam group direction or chord stem direction; fallback geometric heuristic chose wrong side for above-middle-line notes. Fix: thread `explicit_stem_downs` through beam group direction (prefer consistent MusicXML stem over heuristic) and through `position_noteheads()` in `positioner.rs`. All beamed group directions now respect MusicXML `<stem>` when notes in a group agree. Commits: `606e020`
  - [x] T013-C Extend minimum stem length for multi-beam groups (16th/32nd): with 3 beam levels, stems were cramped (only 5u clearance). Fix: add `beam_level_extra = (max_beam_levels − 1) × 5u` to `min_length` at both beam layout phases in `note_layout.rs`. Effect: 8ths=50u (unchanged), 16ths=55u (+5u), 32nds=60u (+10u). Commit: `0cd14bd`
