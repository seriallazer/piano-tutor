...
**Purpose**: Core rendering class that transforms layout engine's computed positions into SVG DOM elements.

### Fields

| Field | Type | Description |
|-------|------|-------------|
| `svg` | SVGSVGElement | Root SVG element for rendering |
| `config` | RenderConfig | Rendering configuration (colors, fonts) |

### Methods

#### render(layout: GlobalLayout, viewport: Viewport): void

**Description**: Main rendering entry point. Removes existing elements, queries visible systems, creates SVG elements.

**Algorithm**:
1. Remove existing system groups: `svg.querySelectorAll('.system').forEach(el => el.remove())`
...
3. For each visible system:
   ...
   - Render glyph runs (SVG <text> elements)
4. Batch insert with DocumentFragment for performance

**Preconditions**: 
- `layout` must be valid GlobalLayout from Feature 016
- `viewport` dimensions must match viewBox coordinate space
- `svg` must be in DOM

**Postconditions**: SVG displays computed layout within viewport bounds
   - Render staff groups (lines, braces, brackets)
